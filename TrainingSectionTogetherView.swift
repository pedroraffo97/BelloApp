//
//  TrainingSectionTogetherView.swift
//  BelloApp
//
...
struct WorkoutSessionTogetherView: View {
        ...
        GeometryReader {
            ...
            VStack(spacing: 20) {
                if isResting {
                    ...
                    //Show all the exercises together
                    ScrollView {
                        VStack(spacing: 16){
                            ForEach(exercises.indices, id: \.self){
                                index in
                                ExerciseDisplay(exerciseName: exercises[index].name, exercisePicture: exercises[index].display, overrideRepetition: repetition)
                                    /*.opacity(index == currentExerciseIndex ? 1.0 : 0.4)
                                    .scaleEffect(index == currentExerciseIndex ? 1.0 : 0.95)
...
struct TrainingSectionTogetherView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            .fullScreenCover(isPresented: $showSession) {
                WorkoutSessionTogetherView(exercises: exercises, repetition: repetition, time: time, resttime: resttime)
